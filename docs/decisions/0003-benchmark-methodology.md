...
Peak RSS uses `/proc/self/status` `VmHWM` inside one isolated child process per benchmark case. The scope is `benchmark-case`, and it includes setup/keygen plus warmup and measured samples. Exactly one RSS scalar is reported per case; it is not duplicated across samples or summarized as a distribution. It is reported as null on platforms where that source is unavailable. CPU and GPU results must never be combined; M2 currently emits CPU only. Long-lived parent-process VmHWM is never labeled as per-case RAM.

The parent process resolves `OPENVM_RUST_TOOLCHAIN` before writing environment metadata or spawning case workers. Publication requires `nightly-2026-01-18`; a generic `nightly` fallback is development-only. The prepared OpenVM context owns the aggregation verifying key so measured samples do not invoke aggregation key generation.
