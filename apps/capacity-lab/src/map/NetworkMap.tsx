...
export function NetworkMap({ result, simTime }: { result: SimulationResult; simTime: number }) {
  ...
  const publishedAt = result.events.find((event) => event.type === "BLOCK_PUBLISHED")?.time ?? result.metrics.proofGate;
  const published = simTime >= publishedAt;
  const blockElapsed = Math.max(0, simTime - publishedAt);
  ...
  const styledPoints = points.map((point) => {
    ...
    const readyAt = result.events.find((event) => event.type === "LOGICAL_READY" && event.slot === point.slot)?.time ?? publishedAt + slot.blockReady;
  ...
  const layers: Layer[] = [
    ...(world ? [new GeoJsonLayer({ id: "world", data: world, filled: true, stroked: true, getFillColor: [24, 38, 58, 205], getLineColor: [112, 145, 184, 45], getLineWidth: 0.7, lineWidthMinPixels: 0.5 })] : []),
